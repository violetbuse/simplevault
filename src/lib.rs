...
pub async fn run_cli() -> ExitCode {
    ...
    let config = if let Some(path) = &args.config_path {
        let resolved = match resolve_config_path(path) {
            ...
            Err(error) => {
                eprintln!("Error resolving config path: {}", error);
    ...
    } else if let Some(env_var) = &args.config_env {
        let delete_after = !args.keep_config;
        match read_config_from_env(env_var, delete_after) {
            Ok(config_value) => config_value,
            Err(error) => {
                eprintln!("Error reading config from env: {}", error);
                return ExitCode::FAILURE;
            }
        }
    } else {
    ...
    if let Some(env_var) = &args.delete_env {
